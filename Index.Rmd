...
base        <- readxl::read_excel("Data/Base_Motor_Pobreza_08052024.xlsx")
...
```{r}
periodo <- base[base$NOMINDICADOR == "Brecha de pobreza a usd 3,20 por día (2011 ppp) (%)"&base$URBANORURALUY=="Total País",]
max<- max(periodo$ANIO)
min<- min(periodo$ANIO)
periodo <- c(min," - ", max)
periodo_formatted <- paste(
...
g1 <- 
  base %>% filter(NOMINDICADOR == "Brecha de pobreza a usd 3,20 por día (2011 ppp) (%)", URBANORURALUY == "Total País") %>%
