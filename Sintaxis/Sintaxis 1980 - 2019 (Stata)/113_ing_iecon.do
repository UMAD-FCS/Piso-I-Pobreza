...
* INDICADOR: Mediana de ingresos per cápita de los hogares (precios corrientes)
* CÓDIGO:    113
...
**MODIFICACIÓN MANUAL: CÓDIGOS Y LOCAL J PARA VARIABLES AUXILIARES**
    ...
    *TOTAL PAÍS
	local i      = 1
	local codind = 113                  // Código de indicador
	...
	tabstat bc_percap_iecon [aw=bc_pesoan] if bc_pe4==1, save s(p50)
	...
	tabstat bc_percap_iecon [aw=bc_pesoan] if bc_pe4==1 & bd_region==`val', save s(p50)
	matrix MATR  [`i',1]=  r(StatTotal) 
	matrix MATR  [`i',2]=  `val'
	matrix MATR  [`i',3]=  `anio'
	local i  = `i' + 1
    ...
    *
	...
	tabstat bc_percap_iecon [aw=bc_pesoan] if bc_pe4==1 & bd_quintilesy==`val', save s(p50)
	matrix MATR  [`i',1]=  r(StatTotal) 
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    ...
    *	
	...
	tabstat bc_percap_iecon [aw=bc_pesoan] if bc_pe4==1 & pobre06==`val', save s(p50)
	matrix MATR  [`i',1]=  r(StatTotal) 
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    ...
    *	
	...
	tabstat bc_percap_iecon [aw=bc_pesoan] if bc_pe4==1 & sexojefe==`val', save s(p50)
	matrix MATR  [`i',1]=  r(StatTotal) 
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
...
xml_tab MATR, save("$tabulados\Auxiliares\AUX113_TP.xls") replace
...
g CODIND                 = 113
g NOMINDICADOR           = "Mediana de ingreso per-cápita de los hogares (Cte. Base diciembre 2006)"
...
************************************************************************************************
    ...
    *PAÍS URBANO
	...
	local j  = 	1
	tabstat bc_percap_iecon [aw=bc_pesoan] if bc_pe4==1 & bc_filtloc==1, save s(p50)
 	matrix MATR  [`i',1]=  r(StatTotal)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
	...
	local j  = 	4
	foreach val of numlist 1/5  {
	tabstat bc_percap_iecon [aw=bc_pesoan] if bc_pe4==1 & bd_quintilesy==`val' & bc_filtloc==1, save s(p50)
	matrix MATR  [`i',1]=  r(StatTotal)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
	...
	local j  = `j' + 1
		...
		local j  = 	11
	foreach val of numlist 1/2  {
	tabstat bc_percap_iecon [aw=bc_pesoan] if bc_pe4==1 & sexojefe==`val' & bc_filtloc==1, save s(p50)
	matrix MATR  [`i',1]=  r(StatTotal)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
...
xml_tab MATR, save("$tabulados\Auxiliares\AUX113_PU.xls") replace
