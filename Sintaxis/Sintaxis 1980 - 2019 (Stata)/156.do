...
* INDICADOR: Personas en hogares sin servicio higiénico de calidad
* CÓDIGO:    154
...
**MODIFICACIÓN MANUAL: CÓDIGOS Y LOCAL J PARA VARIABLES AUXILIARES**
    ...
    *TOTAL PAÍS
	local i      = 1
	local codind = 156                  // Código de indicador
	...
	mean NBI_artefactos11 [aw=bc_pesoan]
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  0
	...
	mean NBI_artefactos11 [aw=bc_pesoan] if bd_region==`val'
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `val'
	matrix MATR  [`i',3]=  `anio'
	local i  = `i' + 1
    ...
    *
	...
	mean NBI_artefactos11 [aw=bc_pesoan] if bc_pe2==`val'
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    ...
    *
	...
	mean NBI_artefactos11 [aw=bc_pesoan] if tramo_edad==`val'
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    ...
    *
	...
	local j  = 	13
	foreach val of numlist 1/2  {
	mean NBI_artefactos11 [aw=bc_pesoan] if bd_e29_1==`val'
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    ...
    *
	...
	mean NBI_artefactos11 [aw=bc_pesoan] if bd_quintilesy==`val'
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    ...
    *
	...
	mean NBI_artefactos11 [aw=bc_pesoan] if pobre06==`val'
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
...
import  excel "C:\Users\Usuario\Dropbox\1. Unidad de Métodos y Acceso a Datos\1. Observatorio UMAD\PISO II_MOTORES\Base web\1. Pobreza y desigualdad\Extracciones parciales\1. Pobreza y desigualdad\Auxiliares\AUX`codind'.xls", firstrow clear
sort AUXILIAR ANIO
drop if ANIO==.

g CODIND                 = 156
g NOMINDICADOR           = "PERSONAS HOGARES SIN ARTEFACTOS BÁSICOS DE CONFORT(% DE LA POBLACIÓN TOTAL)"
g SEXO                   = "NA"
g ASCENDENCIA    		 = "NA"
g QUINTIL       		 = "NA"
g DEPARTAMENTOUY		 = "NA"
...
replace NOMINDICADOR    = "PERSONAS DE 0 A 5 AÑOS EN HOGARES SIN ARTEFACTOS BÁSICOS DE CONFORT (% DE LA POBLACIÓN TOTAL)"     if AUXILIAR==6
replace NOMINDICADOR    = "PERSONAS DE 6 A 12 AÑOS EN HOGARES SIN ARTEFACTOS BÁSICOS DE CONFORT (% DE LA POBLACIÓN TOTAL)"    if AUXILIAR==7
replace NOMINDICADOR    = "PERSONAS DE 13 A 18 AÑOS EN HOGARES SIN ARTEFACTOS BÁSICOS DE CONFORT (% DE LA POBLACIÓN TOTAL)"   if AUXILIAR==8
replace NOMINDICADOR    = "PERSONAS DE 19 A 24 AÑOS EN HOGARES SIN ARTEFACTOS BÁSICOS DE CONFORT (% DE LA POBLACIÓN TOTAL)"   if AUXILIAR==9
replace NOMINDICADOR    = "PERSONAS DE 25 A 29 AÑOS EN HOGARES SIN ARTEFACTOS BÁSICOS DE CONFORT (% DE LA POBLACIÓN TOTAL)"   if AUXILIAR==10
replace NOMINDICADOR    = "PERSONAS DE 30 A 64 AÑOS EN HOGARES SIN ARTEFACTOS BÁSICOS DE CONFORT (% DE LA POBLACIÓN TOTAL)"   if AUXILIAR==11
replace NOMINDICADOR    = "PERSONAS DE 65 AÑOS Y MÁS EN HOGARES SIN ARTEFACTOS BÁSICOS DE CONFORT (% DE LA POBLACIÓN TOTAL)"  if AUXILIAR==12
...
replace NOMINDICADOR    = "PERSONAS EN HOGARES NO EN SITUACIÓN DE POBREZA SIN ARTEFACTOS BÁSICOS DE CONFORT (% DE LA POBLACIÓN TOTAL)"      if AUXILIAR==20
replace NOMINDICADOR    = "PERSONAS EN HOGARES EN SITUACIÓN DE POBREZA SIN ARTEFACTOS BÁSICOS DE CONFORT (% DE LA POBLACIÓN TOTAL)"         if AUXILIAR==21
