...
banco_mundial <- rio::import("Bases/Data_Banco_Mundial/Datos_Pobreza_data.csv") # Descarga de: https://www.bancomundial.org/es/topic/poverty/lac-equity-lab1/poverty/head-count
...
### 126	Brecha de pobreza a usd 2,15 por día (2017 ppp) (%)

#Total País
m126_bm_tp  <- banco_mundial %>% filter(Indicador == "Brecha de pobreza" & `Línea Pobreza` == "Pobreza $2.15 (2017 PPP)" & Area == "Nacional")
...
CODIND       <- 126
NOMINDICADOR <- "Brecha de pobreza a usd 2,15 por día (2017 ppp) (%)"
CATEGORIA    <- "Pobreza"
CORTE_NUEVA  <- "Total"
REGION       <- "Todos"
PAIS         <- "Uruguay"
ANIO         <- m126_bm_tp$Año
VALOR        <- m126_bm_tp$Tasa
RESPONSABLE  <- "JIMENA PANDOLFI"
...
m126_bm_pu  <- banco_mundial %>% filter(Area== "Urbano" & Indicador == "Brecha de pobreza" & `Línea Pobreza` == "Pobreza $2.15 (2017 PPP)")
...
### 127	Brecha de pobreza a usd 3,65 por día (2017 ppp) (%)
 
m127_bm_tp  <- banco_mundial %>% filter(Area== "Nacional" & Indicador == "Brecha de pobreza" & `Línea Pobreza` == "Pobreza $3.65 (2017 PPP)")
...
CODIND       <- 127
NOMINDICADOR <- "Brecha de pobreza a usd 3,65 por día (2017 ppp) (%)"
CATEGORIA    <- "Pobreza"
CORTE_NUEVA  <- "Total"
REGION       <- "Todos"
PAIS         <- "Uruguay"
ANIO         <- m127_bm_tp$Año
VALOR        <- m127_bm_tp$Tasa
RESPONSABLE  <- "JIMENA PANDOLFI"
...
m127_bm_pu  <- banco_mundial %>% filter(Area== "Urbano" & Indicador == "Brecha de pobreza" & `Línea Pobreza` == "Pobreza $3.65 (2017 PPP)")
...
### 128	Brecha de pobreza a usd 6,85 por día (2017 ppp) (%)


m128_bm_tp  <- banco_mundial %>% filter(Area  == "Nacional" & Indicador  == "Brecha de pobreza" & `Línea Pobreza` == "Pobreza $6.85 (2017 PPP)")
...
CODIND       <- 128
NOMINDICADOR <- "Brecha de pobreza a usd 6,85 por día (2017 ppp) (%)"
CATEGORIA    <- "Pobreza"
CORTE_NUEVA  <- "Total"
REGION       <- "Todos"
PAIS         <- "Uruguay"
ANIO         <- m128_bm_tp$Año
VALOR        <- m128_bm_tp$Tasa
RESPONSABLE  <- "JIMENA PANDOLFI"
...
m128_bm_pu  <- banco_mundial %>% filter(Area == "Urbano" & Indicador == "Brecha de pobreza" & `Línea Pobreza` == "Pobreza $6.85 (2017 PPP)")
...
motor_2024 <- rbind(m111, m113, m121, m122, m123, m124, 
...
motor_2024 <- motor_2024  %>% mutate(CORTE = CORTE_NUEVA) 

motor_2024 <- motor_2024 %>%
  rename(REGIÓN = REGION,
         PAÍS = PAIS)

motor_2024 <- motor_2024[c("CODIND", "NOMINDICADOR", "CATEGORIA", "PESTAÑA", "CORTE", "CORTE_NUEVA", "REGIÓN",        
